...
import { sampleWithRequiredData } from '../surah.test-samples';
...
describe('Surah Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.surahs?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to surahService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getSurahIdentifier');
      const id = comp.trackId(entity);
      expect(service.getSurahIdentifier).toHaveBeenCalledWith(entity);
